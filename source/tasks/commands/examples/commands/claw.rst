...
In this example, the design of the claw is composed of two double action solenoids, one on the left and one on the right, hooked up to pneumatic pistons. When the solenoids are set to retract (backward), the claw closes, and then they are set to extend (forward), the claw opens.
